...
class MtfMapperPyTests(unittest.TestCase):
    ...
    def test_web_options_builder(self):
        args = mtf_mapper_py.web_options_to_namespace(
            Path("input.raw"),
            {
                "threshold": 0.4,
    ...
    def test_measurement_summary(self):
        ...
        summary = mtf_mapper_py.summarize_measurements(measurements)
    ...
    def test_edge_profiles_include_lsf_curve_data(self):
        ...
        self.assertEqual(len(esf), len(source_esf))
        self.assertEqual(len(lsf), len(source_esf))
    ...
    @unittest.skipIf(mtf_mapper_py.cv2 is None, "OpenCV is not installed")
    def test_web_preview_detection_and_analysis_payloads(self):
        cv2 = mtf_mapper_py.cv2
        image = np.full((160, 220), 235, dtype=np.uint8)
        rect = ((110, 80), (80, 54), 7)
        points = cv2.boxPoints(rect).astype(np.int32)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            input_path = Path(tmp) / "synthetic.png"
            cv2.imwrite(str(input_path), image)
            options = {"linear": True, "threshold_mode": "hybrid", "threshold": 0.55}
            preview = mtf_mapper_py.web_preview_detection(str(input_path), options)
            self.assertTrue(preview["detection_image"].startswith("data:image/png;base64,"))
            self.assertTrue(preview["threshold_image"].startswith("data:image/png;base64,"))
            self.assertGreaterEqual(len(preview["boxes"]), 1)

            result = mtf_mapper_py.web_analyze(str(input_path), options)
            self.assertGreaterEqual(result["summary"]["edges"], 4)
            self.assertIn("annotated.png", result["outputs"])
            self.assertIn("edge_mtf_values.csv", result["outputs"])
            self.assertIn("edge_sfr_values.csv", result["outputs"])
            self.assertIn("analysis_diagnostics.json", result["outputs"])
            self.assertTrue(result["outputs"]["annotated.png"].startswith("data:image/png;base64,"))
            self.assertEqual(result["measurements"][0]["sfr"][0], 1.0)

    @unittest.skipIf(mtf_mapper_py.cv2 is None, "OpenCV is not installed")
    def test_fastapi_original_preview_endpoint(self):
        from fastapi.testclient import TestClient

        cv2 = mtf_mapper_py.cv2
        image = np.full((40, 60), 220, dtype=np.uint8)
        with tempfile.TemporaryDirectory() as tmp:
            input_path = Path(tmp) / "preview.png"
            cv2.imwrite(str(input_path), image)
            client = TestClient(mtf_mapper_api.app)
            with input_path.open("rb") as fin:
                response = client.post(
                    "/api/original",
                    files={"file": ("preview.png", fin, "image/png")},
                    data={"options": json.dumps({"linear": True})},
                )
